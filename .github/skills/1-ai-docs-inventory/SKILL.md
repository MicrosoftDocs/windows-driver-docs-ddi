...
5. **List all files in the directory on each branch** (one API call per branch, not per file). This detects legacy filename variants (see [Legacy Filename Exceptions](#legacy-filename-exceptions)):
   ...
   # List files on staging branch of published docs repo
   try {
       $r3 = Invoke-RestMethod -Uri "$adoBase/windows-driver-docs-ddi/items?scopePath=wdk-ddi-src/content/{header}/&recursionLevel=OneLevel&versionDescriptor.version=staging&versionDescriptor.versionType=branch&api-version=7.0" -Headers $h
   ...
   c. Check if the file exists on `staging` of the published docs repo (look for the filename in `$pubFiles`).

6. Classify each API into one of three categories:
   - **new**: Stub exists on `stubs/main` but no completed doc on `main`
   - **file exists**: File already exists on `main` (may need update)
...
7. **Output grouped results** to the console for the agent to parse and present:
   ...
   === Doc Exists (may need update) ===
   nf-soundwireclass-otherfunc.md         (function)

   === Stub not found ===
   ne-soundwireclass-some_enum.md         (enumeration)
   ```

8. **Write the updated CSV** at the original CSV path. The format must be compatible with the `ai-docs-generate` skill:

   ```csv
   filename
   wdk-ddi-src/content/{header}/{filename1}.md
   wdk-ddi-src/content/{header}/{filename2}.md
   ```

   The script writes only "new" entries by default. Include "file exists" entries only if the user requested updates.

9. **Output a summary** to the console:
