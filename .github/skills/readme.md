...
The skills.md files for this workflow are located on the repo (and your local clone) in **\wdk-ddi\.github\skills**
...
### MCP Servers (for ai-docs-generate)
...
A user-created CSV listing the APIs to document. Typically exported from the Meriwether internal tracking system. Must contain a column with file paths such as:
...
The header file name (e.g. `soundwireclass.h` or `soundwireclass`). The `.h` extension is stripped automatically.
...
```
/ai-docs-inventory {header}(.h) {path\to\file.csv}
```
...
```
/ai-docs-generate {header}(.h) {path\to\file.csv}
```
...
```
/ai-docs-submit {header}(.h) {path\to\file.csv}
```
...
```
/ai-docs-autopilot {header}(.h) {path\to\file.csv}
```
