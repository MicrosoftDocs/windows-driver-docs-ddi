---
UID: NF:usermode_accessors.ReadWCharFromMode
tech.root: kernel
title: ReadWCharFromMode
ms.date: 07/02/2025
targetos: Windows
description: The ReadWCharFromMode function safely reads a WCHAR value from memory based on the specified processor mode.
prerelease: false
req.assembly: 
...
f1_keywords:
 - ReadWCharFromMode
 - usermode_accessors/ReadWCharFromMode
dev_langs:
 - c++
helpviewer_keywords:
 - ReadWCharFromMode
...
The **ReadWCharFromMode** function safely reads a WCHAR value from memory based on the specified processor mode.

## -parameters

### -param Source

[in] A pointer to the memory location from which to read the WCHAR value.
...
**ReadWCharFromMode** returns the WCHAR value read from the specified memory address.

## -remarks

This function provides a safe way to read a WCHAR value from memory, with extra safety checks when accessing user-mode memory. It ensures that the provided address is valid and accessible based on the specified processor mode.
...
[**WriteWCharToMode**](nf-usermode_accessors-writewchartomode.md)
