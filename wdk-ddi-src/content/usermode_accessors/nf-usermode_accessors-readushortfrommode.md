...
f1_keywords:
 - ReadUShortFromMode
 - usermode_accessors/ReadUShortFromMode
dev_langs:
 - c++
helpviewer_keywords:
 - ReadUShortFromMode
...
The **ReadUShortFromMode** function safely reads a USHORT value from memory based on the specified processor mode.

## -parameters

### -param Source

[in] A pointer to the memory location from which to read the USHORT value.
...
| Value | Meaning |
| ----- | ------- |
| **KernelMode** | **Source** points to kernel-mode memory. The function performs a direct read from the specified address. |
| **UserMode** | **Source** points to user-mode memory. The function performs a safe read with appropriate checks to ensure the memory is accessible. |

## -returns

**ReadUShortFromMode** returns the USHORT value read from the specified memory address.

## -remarks

This function provides a safe way to read a USHORT value from memory, with extra safety checks when accessing user-mode memory. It ensures that the provided address is valid and accessible based on the specified processor mode.
