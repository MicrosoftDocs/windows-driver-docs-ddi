...
ms.date: 07/02/2025
targetos: Windows
description: The CopyFromModeAligned function safely copies data from specified-mode memory to kernel memory, with alignment checking.
prerelease: false
req.assembly: 
...
The **CopyFromModeAligned** function safely copies data from specified-mode memory to kernel memory, with alignment checking.
...
| Value | Meaning |
| ----- | ------- |
| **KernelMode** | **Source** points to kernel-mode memory. The function performs a direct memory copy with [memory_order_relaxed semantics](/cpp/standard-library/atomic-enums?view=msvc-170#memory_order_enum). |
...
The function raises a structured exception if the copy operation fails, such as when the source address is not valid for the specified mode, is not properly aligned according to the **Alignment** parameter, or is inaccessible.
