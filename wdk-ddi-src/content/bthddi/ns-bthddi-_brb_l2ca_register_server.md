...
After the profile driver registers itself, it should then 
    <a href="/previous-versions/ff536657(v=vs.85)">build and send</a> a 
    <a href="/previous-versions/ff536621(v=vs.85)">BRB_REGISTER_PSM</a> request so the Bluetooth
...
After the profile driver has registered itself, the Bluetooth driver stack can notify it when a remote
    ...
    <a href="/windows-hardware/drivers/ddi/bthddi/nc-bthddi-pfnbthport_indication_callback">L2CAP Callback Function</a> that the
    profile driver implements and specifies in the 
    <b>IndicationCallback</b> member.

For more information about L2CAP servers and PSMs, see 
    <a href="/previous-versions/ff536572(v=vs.85)">Accepting
...
While this procedure allows a profile driver to accept incoming connection requests, it does not
    ...
    <a href="/previous-versions/ff536671(v=vs.85)">Communicating with SDP
    Servers</a>.
