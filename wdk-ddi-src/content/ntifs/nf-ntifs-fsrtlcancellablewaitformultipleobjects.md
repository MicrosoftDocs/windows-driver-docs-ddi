...
<td width="60%">
The wait was interrupted by a pending cancel request on the specified IRP.  Note that this value is returned only if a valid IRP is passed to <b>FsRtlCancellableWaitForMultipleObjects</b> and the IRP was canceled by <a href="/windows/win32/api/ioapiset/nf-ioapiset-cancelsynchronousio">CancelSynchronousIo</a>.
...
If either <i>Count</i> is greater than MAXIMUM_WAIT_OBJECTS or if <i>WaitBlockArray</i> is <b>NULL</b> and Count is greater than THREAD_WAIT_OBJECTS, the system issues <a href="/windows-hardware/drivers/debugger/bug-check-0xc--maximum-wait-objects-exceeded">Bug Check 0xC: MAXIMUM_WAIT_OBJECTS_EXCEEDED</a>.
