...
**ZwQueryDirectoryFileEx** returns STATUS_SUCCESS or an appropriate error status. The set of error status values that can be returned is file system-specific. **ZwQueryDirectoryFileEx** also returns the number of bytes actually written to the given **FileInformation** buffer in the **Information** member of **IoStatusBlock**. See [**NtQueryDirectoryFileEx**](nf-ntifs-ntquerydirectoryfileex.md) for a list of some possible error codes.
