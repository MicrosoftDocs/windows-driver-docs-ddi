...
This structure is used in an [NDIS_STATUS_WWAN_MPDP_LIST](/windows-hardware/drivers/network/ndis-status-wwan-mpdp-list) status indication, which is sent as an asynchronous response to an [OID_WWAN_MPDP](/windows-hardware/drivers/network/oid-wwan-mpdp) query request.
