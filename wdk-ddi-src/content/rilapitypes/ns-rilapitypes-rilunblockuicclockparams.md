---
UID: NS:rilapitypes.RILUNBLOCKUICCLOCKPARAMS
title: RILUNBLOCKUICCLOCKPARAMS (rilapitypes.h)
description: "Microsoft reserves the RILUNBLOCKUICCLOCKPARAMS structure for internal use only. Don't use this structure in your code."
old-location: netvista\rilunblockuicclockparams.htm
tech.root: netvista
ms.date: 05/02/2018
keywords: ["RILUNBLOCKUICCLOCKPARAMS structure"]
ms.keywords: "*LPRILUNBLOCKUICCLOCKPARAMS, RILUNBLOCKUICCLOCKPARAMS, RILUNBLOCKUICCLOCKPARAMS structure [Network Drivers Starting with Windows Vista], netvista.rilunblockuicclockparams, ntddrilapitypes/RILUNBLOCKUICCLOCKPARAMS"
req.header: rilapitypes.h
req.include-header: Rilapitypes.h
req.target-type: Windows
req.target-min-winverclnt: 
req.target-min-winversvr: 
req.kmdf-ver: 
req.umdf-ver: 
req.ddi-compliance: 
req.unicode-ansi: 
req.idl: 
req.max-support: 
req.namespace: 
req.assembly: 
req.type-library: 
req.lib: 
req.dll: 
req.irql: 
targetos: Windows
req.typenames: RILUNBLOCKUICCLOCKPARAMS, *LPRILUNBLOCKUICCLOCKPARAMS
f1_keywords:
 - RILUNBLOCKUICCLOCKPARAMS
 - rilapitypes/RILUNBLOCKUICCLOCKPARAMS
 - LPRILUNBLOCKUICCLOCKPARAMS
 - rilapitypes/LPRILUNBLOCKUICCLOCKPARAMS
topic_type:
 - APIRef
 - kbSyntax
api_type:
 - HeaderDef
api_location:
 - ntddrilapitypes.h
api_name:
 - RILUNBLOCKUICCLOCKPARAMS
 - LPRILUNBLOCKUICCLOCKPARAMS
---

# RILUNBLOCKUICCLOCKPARAMS structure (rilapitypes.h)


## -description

This topic supports the Windows driver infrastructure and is not intended to be used directly from your code.

## -struct-fields

### -field lockCredential

### -field szNewPassword

