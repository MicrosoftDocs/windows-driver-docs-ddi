---
UID: NS:rilapitypes.RILCALLFORWARDINGSETTINGS
title: RILCALLFORWARDINGSETTINGS (rilapitypes.h)
description: "Don't use this structure in your code. Microsoft reserves the RILCALLFORWARDINGSETTINGS structure for internal use only."
old-location: netvista\rilcallforwardingsettings.htm
tech.root: netvista
ms.date: 05/02/2018
keywords: ["RILCALLFORWARDINGSETTINGS structure"]
ms.keywords: "*LPRILCALLFORWARDINGSETTINGS, RILCALLFORWARDINGSETTINGS, RILCALLFORWARDINGSETTINGS structure [Network Drivers Starting with Windows Vista], netvista.rilcallforwardingsettings, ntddrilapitypes/RILCALLFORWARDINGSETTINGS"
req.header: rilapitypes.h
req.include-header: Rilapitypes.h
req.target-type: Windows
req.target-min-winverclnt: 
req.target-min-winversvr: 
req.kmdf-ver: 
req.umdf-ver: 
req.ddi-compliance: 
req.unicode-ansi: 
req.idl: 
req.max-support: 
req.namespace: 
req.assembly: 
req.type-library: 
req.lib: 
req.dll: 
req.irql: 
targetos: Windows
req.typenames: RILCALLFORWARDINGSETTINGS, *LPRILCALLFORWARDINGSETTINGS
f1_keywords:
 - RILCALLFORWARDINGSETTINGS
 - rilapitypes/RILCALLFORWARDINGSETTINGS
 - LPRILCALLFORWARDINGSETTINGS
 - rilapitypes/LPRILCALLFORWARDINGSETTINGS
topic_type:
 - APIRef
 - kbSyntax
api_type:
 - HeaderDef
api_location:
 - ntddrilapitypes.h
api_name:
 - RILCALLFORWARDINGSETTINGS
 - LPRILCALLFORWARDINGSETTINGS
---

# RILCALLFORWARDINGSETTINGS structure (rilapitypes.h)


## -description

This topic supports the Windows driver infrastructure and is not intended to be used directly from your code.

## -struct-fields

### -field cbSize

### -field dwParams

### -field dwStatus

### -field dwInfoClasses

### -field raAddress

### -field rsaSubAddress

### -field dwDelayTime

