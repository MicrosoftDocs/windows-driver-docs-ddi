...
Starting with zero, NetAdapterCx assigns an unique identifier value for each queue that it creates. The client driver specifies the number of receive queues that the network adapter supports in the NumRxQueues member of the NET_ADAPTER_DATAPATH_CAPABILITIES structure. The client driver passes this structure to NetAdapterSetDataPathCapabilities. Identifier values range from zero to the value of NumTxQueues plus NumRxQueues minus one.

The minimum NetAdapterCx version for **NetRxQueueInitGetQueueId** is 1.0.
