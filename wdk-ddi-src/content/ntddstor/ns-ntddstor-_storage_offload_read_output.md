...
The value for **Token** is generated by the copy provider of the storage device. The data set ranges included when the [DEVICE_DSM_INPUT](./ns-ntddstor-_device_manage_data_set_attributes.md) structure was sent in the [IOCTL_STORAGE_MANAGE_DATA_SET_ATTRIBUTES](./ni-ntddstor-ioctl_storage_manage_data_set_attributes.md) control code request are marked by **Token**. The data set ranges associated with **Token** are available for subsequent write requests while the duration in the **TimeToLive** member of [DEVICE_DSM_OFFLOAD_READ_PARAMETERS](./ns-ntddstor-_device_dsm_offload_read_parameters.md) is valid.
