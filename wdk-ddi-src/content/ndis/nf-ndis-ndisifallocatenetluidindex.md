...
The 
  ...
  <a href="/windows/win32/api/ifdef/ns-ifdef-net_luid_lh">NET_LUID</a> index for an NDIS network interface
  provider.
...
A pointer to a caller-supplied 
     <a href="/windows/win32/api/ifdef/ns-ifdef-net_luid_lh">NET_LUID</a> index variable. If allocation is
     successful, 
...
NDIS interface providers call the 
    ...
    <a href="/windows/win32/api/ifdef/ns-ifdef-net_luid_lh">NET_LUID</a> index. An interface provider must allocate
...
<b>NdisIfAllocateNetLuidIndex</b> attempts to allocate a 24-bit NET_LUID index that is unique to the local
    ...
    <a href="/windows-hardware/drivers/ddi/ndis/nf-ndis-ndisiffreenetluidindex">NdisIfFreeNetLuidIndex</a> function to
    free the index.

To build a 
    <a href="/windows/win32/api/ifdef/ns-ifdef-net_luid_lh">NET_LUID</a> value from the NET_LUID index and the
    interface type, an interface provider calls the 
    <a href="/windows-hardware/drivers/network/ndis-make-net-luid">NDIS_MAKE_NET_LUID</a> macro.
...
The interface provider must store the NET_LUID values that it allocates in persistent storage. For
    ...
    <a href="/windows-hardware/drivers/ddi/ndis/nf-ndis-ndisiffreenetluidindex">NdisIfFreeNetLuidIndex</a> later to
...
<a href="/windows-hardware/drivers/ddi/ndis/nf-ndis-ndisiffreenetluidindex">NdisIfFreeNetLuidIndex</a>
