...
> [!WARNING]
> The *MiniportResetEx* and [*MiniportCheckForHangEx*](nc-ndis-miniport_check_for_hang.md) callback functions are discouraged for all NDIS 6.83 and later drivers. For more information, see [Check-for-Hang and Reset operations in NDIS 6.83 and later](/windows-hardware/drivers/network/miniport-adapter-check-for-hang-and-reset-operations#check-for-hang-and-reset-operations-in-ndis-683-and-later).


NDIS calls an NDIS miniport driver's 
   <i>MiniportResetEx</i> function to initiate a reset of a network interface card (NIC). For more information, see <a href="/windows-hardware/drivers/network/miniport-adapter-check-for-hang-and-reset-operations">Miniport Adapter Check-for-Hang and Reset Operations</a> and <a href="/windows-hardware/drivers/network/hardware-reset">Miniport Driver Hardware Reset</a>.
...
<a href="/windows-hardware/drivers/ddi/ndis/nc-ndis-miniport_reset">MiniportResetEx</a> successfully reset the miniport adapter to an operational state.
...
The driver will complete the reset operation asynchronously by calling the 
       <a href="/windows-hardware/drivers/ddi/ndis/nf-ndis-ndismresetcomplete">NdisMResetComplete</a> function when the
       operation is done.
...
<a href="/windows-hardware/drivers/ddi/ndis/nc-ndis-miniport_reset">MiniportResetEx</a> determined that the miniport adapter was currently being reset, so this call is
       superfluous.
...
If a reset operation changes other information, such as multicast or functional addressing
    ...
    <a href="/windows-hardware/drivers/network/oid-pnp-add-wake-up-pattern">OID_PNP_ADD_WAKE_UP_PATTERN</a>. The
...
NDIS will not abort any pending OID requests or send requests. If the driver can safely complete
    ...
    by calling the 
    <a href="/windows-hardware/drivers/ddi/ndis/nf-ndis-ndismoidrequestcomplete">NdisMOidRequestComplete</a> or 
...
<div class="alert"><b>Note</b>  Starting with NDIS 6.30, the miniport driver must not hold pending OID or send requests until after
    ...
    by calling the 
    <a href="/windows-hardware/drivers/ddi/ndis/nf-ndis-ndismoidrequestcomplete">NdisMOidRequestComplete</a> or 
    <a href="/windows-hardware/drivers/ddi/ndis/nf-ndis-ndismsendnetbufferlistscomplete">NdisMSendNetBufferListsComplete</a> functions respectively  before 
    the reset operation is complete. If the driver completes the reset operation asynchronously, it must complete pending OID and send requests before the driver calls <a href="/windows-hardware/drivers/ddi/ndis/nf-ndis-ndismresetcomplete">NdisMResetComplete</a>.</div>
<div> </div>
A miniport driver should not call the 
    <a href="/windows-hardware/drivers/ddi/ndis/nf-ndis-ndismindicatestatusex">NdisMIndicateStatusEx</a> function to
...
If 
    ...
    call the 
    <a href="/windows-hardware/drivers/ddi/ndis/nf-ndis-ndisstallexecution">NdisStallExecution</a> function. However, a    
...
If 
    ...
    <a href="/windows-hardware/drivers/ddi/ndis/nf-ndis-ndismresetcomplete">NdisMResetComplete</a> function.
...
If a miniport adapter is in the 
    ...
    <a href="/windows-hardware/drivers/network/ndis-status-link-state">NDIS_STATUS_LINK_STATE</a> status
...
NDIS calls the 
    <a href="/windows-hardware/drivers/ddi/ndis/nc-ndis-miniport_check_for_hang">MiniportCheckForHangEx</a> function
    ...
    NdisMSetMiniportAttributes</a> function during initialization. For more information, see <a href="/windows-hardware/drivers/network/miniport-adapter-check-for-hang-and-reset-operations">Miniport Adapter Check-for-Hang and Reset Operations</a>.
...
NDIS cannot determine whether a NIC has stopped responding for receive operations. To handle this kind
    ...
    force a reset. A miniport can also request a reset by calling <a href="/windows-hardware/drivers/ddi/ndis/nf-ndis-ndismresetminiport">NdisMResetMiniport</a>.
...
<a href="/windows-hardware/drivers/network/hardware-reset">Miniport Driver Hardware Reset</a>
...
<a href="/windows-hardware/drivers/ddi/ndis/nf-ndis-ndismresetcomplete">NdisMResetComplete</a>



<a href="/windows-hardware/drivers/ddi/ndis/nf-ndis-ndismresetminiport">NdisMResetMiniport</a>
