---
UID: NC:uart.UART_SET_POWER_D0
tech.root: uart
title: UART_SET_POWER_D0
ms.date: 03/06/2025
targetos: Windows
description: Supplies a function to transition the UART to D0.
prerelease: false
req.assembly: 
req.construct-type: function
req.ddi-compliance: 
req.dll: 
req.header: uart.h
req.idl: 
req.include-header: 
req.irql: 
req.kmdf-ver: 
req.lib: 
req.max-support: 
req.namespace: 
req.redist: 
req.target-min-winverclnt: 
req.target-min-winversvr: 
req.target-type: 
req.type-library: 
req.umdf-ver: 
req.unicode-ansi: 
topic_type:
 - apiref
api_type:
 - LibDef
api_location:
 - uart.h
api_name:
 - UART_SET_POWER_D0
f1_keywords:
 - UART_SET_POWER_D0
 - uart/UART_SET_POWER_D0
dev_langs:
 - c++
helpviewer_keywords:
 - UART_SET_POWER_D0
---

# UART_SET_POWER_D0 function (uart.h)

## -description

Supplies a function to transition the UART to D0.

## -parameters

### -param Port [_In_]

A pointer to a [**_CPPORT**](ns-uart-_cpport.md) structure that contains the  address of the port object that describes the UART hardware.

## -returns

## -remarks

## -see-also

[**UART_HARDWARE_DRIVER**](ns-uart-_uart_hardware_driver.md)

[**_CPPORT**](ns-uart-_cpport.md)

[**uart.h**](index.md)