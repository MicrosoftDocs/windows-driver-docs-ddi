...
ms.date: 09/16/2021
targetos: Windows
description: TBD - EVT_ACX_VOLUME_RETRIEVE_LEVEL tells the driver that a request to retrieve the volume level has made???.
prerelease: true
req.assembly: 
...
TBD - EVT_ACX_VOLUME_RETRIEVE_LEVEL tells the driver that a request to retrieve the volume level has made???.
