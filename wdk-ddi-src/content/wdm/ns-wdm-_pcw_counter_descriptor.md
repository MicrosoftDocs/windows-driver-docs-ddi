...
An array of `PCW_COUNTER_DESCRIPTOR` structures is provided during counterset registration (via the [PCW_REGISTRATION_INFORMATION](ns-wdm-_pcw_registration_information.md) structure passed to `PcwRegister`). The information from the array is recorded and will be used by `PcwCreateInstance` or `PcwAddInstance` to extract counter values from the structures described by a `PCW_DATA` array.
