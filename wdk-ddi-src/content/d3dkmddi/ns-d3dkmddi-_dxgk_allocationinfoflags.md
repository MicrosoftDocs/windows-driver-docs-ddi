...
[out] A UINT value that specifies whether the allocation is swizzled and requires special support by the video memory manager to lock the allocation. For more information about swizzled allocations, see [Locking Swizzled Allocations](https://docs.microsoft.com/windows-hardware/drivers/display/locking-swizzled-allocations).
