...
Any other display miniport driver function that shares memory (for example, some portion of the state represented by <i>MiniportDeviceContext</i>) with <i>DxgkDdiInterruptRoutine</i> must call <a href="/windows-hardware/drivers/ddi/dispmprt/nc-dispmprt-dxgkcb_synchronize_execution">DxgkCbSynchronizeExecution</a> to synchronize its access to the shared memory.

The <i>DxgkDdiInterruptRoutine</i> function can call <a href="/windows-hardware/drivers/ddi/dispmprt/nc-dispmprt-dxgkcb_queue_dpc">DxgkCbQueueDpc</a> and <a href="/windows-hardware/drivers/ddi/d3dkmddi/nc-d3dkmddi-dxgkcb_notify_interrupt">DxgkCbNotifyInterrupt</a> but must not call any other <b>DxgkCbXxx</b> functions. For more information on the proper sequence of function calls, see <a href="/windows-hardware/drivers/display/submitting-a-command-buffer">Submitting a Command Buffer</a>.
